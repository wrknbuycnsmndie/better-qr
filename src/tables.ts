...
const ECC_CODEWORDS_PER_BLOCK: Record<ErrorCorrectionLevel, readonly number[]> = {
  ...
  Q: [
    ...
    30, 28, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30,
  ],
  H: [
...
export function canFitDataBits(
  dataBits: number,
  version: number,
...
export function getDataCapacityBits(
  version: number,
  errorCorrectionLevel: ErrorCorrectionLevel,
): number {
...
export function getNumDataCodewords(
  ...
  const { rawCodewords, blockEccLength, numBlocks } = getErrorCorrectionBlockLayout(
    version,
