...
export const ECC_CODEWORDS_PER_BLOCK: Record<ErrorCorrectionLevel, readonly number[]> = {
...
export const NUM_ERROR_CORRECTION_BLOCKS: Record<ErrorCorrectionLevel, readonly number[]> = {
...
export function getNumDataCodewords(version: number, errorCorrectionLevel: ErrorCorrectionLevel): number {
  const eccCodewords = ECC_CODEWORDS_PER_BLOCK[errorCorrectionLevel][version];
  const blocks = NUM_ERROR_CORRECTION_BLOCKS[errorCorrectionLevel][version];

  if (eccCodewords === undefined || blocks === undefined) {
    throw new RangeError(`Invalid QR version ${version}`);
  }

  return Math.floor(getNumRawDataModules(version) / 8) - eccCodewords * blocks;
}
