...
describe("validation", () => {
  ...
  test("rejects oversized payloads within a selected range", () => {
    ...
    expect(() => createQrCode("x".repeat(100), { minVersion: 1, maxVersion: 1, errorCorrectionLevel: "L" })).toThrow(/does not fit/);
  });
  ...
  test("rejects invalid SVG render options", () => {
    ...
    expect(() => toSvg("x", { icon: { href: "./logo.png", paddingRatio: -0.1 } })).toThrow(/paddingRatio/);
    expect(() => toSvg("x", { icon: { href: "./logo.png", paddingRatio: 0.25 } })).toThrow(/paddingRatio/);
